...
test "reject coalesce" {
    ...
    var a_node = Node.init(a, .{ .type = .gp, .count = 1 }, alloc);
    try a_node.placeNode(p);
    try a_node.placeNode(q);
    var b_node = Node.init(b, .{ .type = .gp, .count = 1 }, alloc);
    try b_node.placeNode(r);
    var p_node = Node.init(p, .{ .type = .gp, .count = 1 }, alloc);
    try p_node.placeNode(a);
    try p_node.placeNode(q);
    try p_node.placeNode(r);
    var q_node = Node.init(q, .{ .type = .gp, .count = 1 }, alloc);
    try q_node.placeNode(p);
    try q_node.placeNode(a);
    try q_node.placeNode(r);
    var r_node = Node.init(r, .{ .type = .gp, .count = 1 }, alloc);
